...
import time
import pandas as pd
from parameters import REPORT_PATH, CURRENT_LOCATION

df_inferred_faces = pd.DataFrame(columns=['name/id','time','date','location','face_distance','class-detected'])


def store_inferred_face_in_dataframe( name_of_person, face_distance, label):
    '''
    ...
    if name_of_person not in df_inferred_faces['name/id'].values:        
        ...
        df_inferred_faces.loc[len(df_inferred_faces)] = [name_of_person, current_time, current_date, current_location, face_distance, label]
